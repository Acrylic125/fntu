import fs from "fs";
import { LocationsRawDataSchema, LocationsSchema } from "./schema";
import { z } from "zod";

const MappingSchema = z.record(z.string(), z.string());

function doMapping(
  locations: z.infer<typeof LocationsSchema>,
  mappings: Record<string, string>
): z.infer<typeof LocationsSchema> {
  return locations.map((location) => {
    const altNames: string[] = [...location.altNames];
    if (location.identifier) {
      const mapping = mappings[location.identifier];
      if (mapping) {
        altNames.push(mapping);
      }
    }
    return {
      ...location,
...
export async function transformLocations(
  locationsRawDataPath: string,
  outputPath: string,
  options: {
    schoolFacilitiesMappingsPath: string;
    mainFacilitiesMappingsPath: string;
  }
) {
  const schoolFacilitiesMappings = await MappingSchema.parseAsync(
    JSON.parse(
      await fs.promises.readFile(options.schoolFacilitiesMappingsPath, "utf8")
    )
  );
  const mainFacilitiesMappings = await MappingSchema.parseAsync(
    JSON.parse(
      await fs.promises.readFile(options.mainFacilitiesMappingsPath, "utf8")
    )
  );
  const locationsRawData = await LocationsRawDataSchema.parseAsync(
    JSON.parse(await fs.promises.readFile(locationsRawDataPath, "utf8"))
  );

  let locations = locationsRawData.map((location) => ({
    ...location,
    altNames: location.infos.map((info) => info.name),
  }));
  const mergedMappings = {
    ...schoolFacilitiesMappings,
    ...mainFacilitiesMappings,
  };
  locations = doMapping(locations, mergedMappings);

  // Reprocess locations to remove altName duplicates.
  locations = locations.map((location) => {
    return {
      ...location,
      altNames: Array.from(new Set(location.altNames)),
    };
  });
